...
- **A timeline layered the way Frigate's own reads** — a white motion histogram over translucent bands, amber for a detection and red for an alert.
...
While a card is live and on screen it re-reads the last five minutes of review data every five seconds — the only polling the card does, and what lets the strip show activity that happened after the page was opened.
...
`frigate_stream: auto` (the default) decides by device. A machine with a mouse and a screen at least 1024 points wide gets the full stream — it has the headroom, and fullscreen is then already at full quality the moment it opens, with no reconnect. Everything else gets the sub stream: on the same dashboard that is 54 Mpx/s against 510, and an eighth of the bandwidth, with nothing visibly lost at the size a phone shows a card. `main` and `sub` force the choice; a tablet wanting full quality in fullscreen is the case for `main`.
